...
RSpec.describe 'ツイート編集', type: :system do
  ...
  context 'ツイート編集ができるとき' do
    it 'ログインしたユーザーは自分が投稿したツイートの編集ができる' do
      ...
      # 投稿内容を編集する
      @tweet1.text = Faker::Lorem.sentence
      @tweet1.image = Faker::Lorem.sentence
      fill_in 'tweet[image]', with: @tweet1.image
      fill_in 'tweet[text]', with: @tweet1.text
...
RSpec.describe 'ツイート削除', type: :system do
  ...
  context 'ツイート削除ができるとき' do
    it 'ログインしたユーザーは自らが投稿したツイートの削除ができる' do
      ...
      expect(page).to have_no_selector ".content_post[sytle='background-image: url(#{@tweet1.image});']"
...
RSpec.describe 'ツイート詳細', type: :system do
  ...
  it 'ログインしたユーザーはツイート詳細ページに遷移してコメント投稿欄が表示される' do
    ...
    # 詳細ページに遷移する
    all('.more')[0].hover.find_link('詳細', href: tweet_path(@tweet)).click
    ...
    # コメント用のフォームが存在する
    expect(page).to have_selector "form[action='/tweets/#{@tweet.id}/comments']"
  end
  it 'ログインしていない状態でツイート詳細ページに遷移できるもののコメント投稿欄が表示されない' do
    ...
    expect(page).to have_content @tweet.text
    # フォームが存在しないことを確認する
    expect(page).to have_no_selector "form[action='/tweets/#{@tweet.id}/comments']"
